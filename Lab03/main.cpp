...
The program has the ability to create threads and to set their priority (without synchronization)
	...
	while (1)
	{
		...
		case 1:
		{
			...
			if (thread_num > 0)
			{
				...
				{
					cout << "Thread " << i << endl;
					cout << "Priority: " << out[i].tPriority << endl;
...
void CreateThreadsF(int thread_num, unsigned long long interval_start, unsigned long long interval_end, int* arr, DWORD priority)
{
	...
	for (int i = 0; i < thread_num; i++)
	{
		...
		if (hThreads[i] == NULL)
		{
			cout << "Error while thread creating: " << GetLastError() << endl;
			exit(1);
		}
		...
		out[i].tPriority = GetPriorityClass(hThreads[i]);
	}
